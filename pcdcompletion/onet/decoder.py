...
class DecoderBatchNorm(nn.Module):
    ...
    def __init__(self, dim=3, z_dim=128, c_dim=128,
                 hidden_size=256, leaky=False, bnm=.1):
        super().__init__()
        self.z_dim = z_dim
        self.c_dim = c_dim
        ...
        if self.c_dim != 0:
            self.fc_c = nn.Linear(c_dim, hidden_size)
        self.fc_p = nn.Conv1d(dim, hidden_size, 1)
        self.block0 = ResnetBlockConv1d(hidden_size, bnm=bnm)
        self.block1 = ResnetBlockConv1d(hidden_size, bnm=bnm)
        self.block2 = ResnetBlockConv1d(hidden_size, bnm=bnm)
        self.block3 = ResnetBlockConv1d(hidden_size, bnm=bnm)
        self.block4 = ResnetBlockConv1d(hidden_size, bnm=bnm)

        self.bn = nn.BatchNorm1d(hidden_size, momentum=bnm)
    ...
    def forward(self, p, z, c, **kwargs):
        ...
        if self.c_dim != 0:
            ...
            net = net + net_c
            # net = torch.cat((net, c[:, None].repeat()), dim=-1)
            # net = self.fc(net)
